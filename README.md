...
└─ src/SsamMobileApi/
   ...
   ├─ Controllers/
   │  └─ DistributorsController.cs      Sample REST endpoints
   ├─ Services/
   │  ├─ IDistributorService.cs         Business-logic contract
   │  └─ DistributorService.cs          Implementation (only place touching the DB)
   ├─ Models/
   │  └─ DistributorDtos.cs             Request/response shapes (public contract)
   ├─ Data/
   │  ├─ AppDbContext.cs                EF Core context (regenerated by scaffold)
   │  ├─ AppDbContextFactory.cs         Design-time factory for `dotnet ef`
   │  └─ Entities/Distributor.cs        Sample entity (replaced by scaffold)
   └─ Infrastructure/
      └─ GlobalExceptionHandler.cs      Turns exceptions into clean problem+json
```

The `Distributor*` files are a **working placeholder** so the project compiles
and runs. You replace them with types generated from the real database
(section 4).
...
This regenerates `Data/AppDbContext.cs` and one class per table under
`Data/Entities/`. Notes:

- `--no-onconfiguring` keeps the connection string out of the generated code
  (it stays in config, wired up in `Program.cs`).
- `--force` overwrites previous output. Re-run it whenever the DB schema changes.
- Limit tables with repeated `--table dbo.Distributors --table dbo.Orders` if you
  only need part of a large database.
- After scaffolding, delete the placeholder `Data/Entities/Distributor.cs` if the
  real DB doesn't have that exact table, and update `DistributorService` /
  `DistributorsController` / DTOs to match real types.

The database stays the source of truth — this API does **not** run migrations
against it.
...
- **Scopes/roles**: `Program.cs` shows a `ReadAccess` policy checking the `scp`
  (delegated) or `roles` (app) claim for `Distributors.Read`. Apply it with
...
1. **API app registration** (this service)
   - *Expose an API* → set Application ID URI `api://<clientId>` → add a scope,
     e.g. `Distributors.Read`.
   - Optionally *App roles* for app-only (client-credentials) access from
     OutSystems, e.g. role value `Distributors.Read`.
